...
//		return result;
	...
	jQuery.views.helpers({
	    timeago: function (createdAt) {
              if (typeof(createdAt)=='undefined' || !createdAt) return '';
	      // using timeago plugin (issues with UTC date and L10N)
	      // Date.parseInvariant is exposed by Ajax 4
              t = jQuery.timeago(createdAt);
              return '<time class="timeago" datetime="'+createdAt+'">' + t + '</time>';
	      return '<span class="time" data-time="'+createdAt+'">' + t + '</span>';
	    }
	});
